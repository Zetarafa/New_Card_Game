document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateSelectedCards() {
        ...
        selectedCards.forEach((cardData, index) => {
            ...
            miniCard.style.position = "relative"; // Asegurar que el nombre esté dentro
            miniCard.style.display = "flex";
            miniCard.style.alignItems = "center";
            miniCard.style.justifyContent = "center";
            miniCard.style.color = "white"; // Asegurar que el texto sea visible
            miniCard.style.fontSize = "12px";
            miniCard.style.fontWeight = "bold";
            miniCard.style.textAlign = "center";
            miniCard.style.textShadow = "1px 1px 2px black"; // Mejor visibilidad

            // Añadir nombre dentro de la minicarta
    ...
    cards.forEach(card => {
        card.addEventListener("click", () => {
            ...
            const cardData = {
                id: cardId,
                backgroundImage: card.style.backgroundImage.replace('url("', '').replace('")', ''),
                name: cardName,
            };

            if (isNewOldPage) {
                selectedCards = selectedCards.filter(c => !["1", "2"].includes(c.id));
